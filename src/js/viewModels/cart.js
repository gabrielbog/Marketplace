 ...
 */
/*
 * Your cart ViewModel code goes here
 */
...
"ojs/ojknockout", "ojs/ojselectcombobox", "ojs/ojlabel", "ojs/ojvalidationgroup", "ojs/ojformlayout", "ojs/ojtoolbar", "ojs/ojmessages", "ojs/ojtable"],
  function(accUtils, ko, ArrayDataProvider, BufferingDataProvider, ojkeyset_1, NumberConverter, records) {
    function CartViewModel() {
      ...
      this.transformedData = this.productData.map(function (item) {
        return {
          value: item.id,
          label: item.name
        };
      });
      this.productProvider = new ArrayDataProvider(this.transformedData, {
          keyAttributes: 'value'
      });
      
      this.cartObservableArray = ko.observableArray([]);
      this.dataprovider = new BufferingDataProvider(new ArrayDataProvider(this.cartObservableArray, {
          keyAttributes: 'id'
      }));
      ...
      this.inputProductId = ko.observable(1);
      ...
      this.addRow = () => {
          if (this.groupValid() !== 'invalidShown') {
              var prod = null;
              for (const entry of this.productData) {
                if (entry.id == this.inputProductId())
                  prod = entry;
              }
      ...
      this.updateRow = () => {
          if (this.groupValid() !== 'invalidShown') {
            ...
            if (currentRow != null) {
                ...
                var prod = null;
                for (const entry of this.productData) {
                  if (entry.id == dataObj.data.id)
                    prod = entry;
                }

                if (prod) {
                  const currentQuantity = dataObj.data.quantity + 1;
                  const key = dataObj.key;
                  const newData = {
                    id: dataObj.data.id,
                    name: dataObj.data.name,
                    quantity: currentQuantity,
                    price: prod.price * currentQuantity
                  };
      ...
      this.removeRow = () => {
          ...
          if (currentRow != null) {
              const dataObj = element.getDataForVisibleRow(currentRow.rowIndex);
              if (dataObj.data.quantity < 2) {
                  this.totalPrice(this.totalPrice() - dataObj.data.price);
      ...
      this.findIndex = (key) => {
          const ar = this.cartObservableArray();
          for (let idx = 0; idx < this.cartObservableArray().length; idx++) {
              if (ar[idx].id === key) {
                  return idx;
              }
          }
      ...
      this.commitOneRow = (editItem) => {
          ...
          if (idx > -1) {
              if (editItem.operation === 'update') {
                  this.cartObservableArray.splice(idx, 1, editItem.item.data);
              }
              else if (editItem.operation === 'remove') {
                  this.cartObservableArray.splice(idx, 1);
              }
              else {
                  error = {
                      severity: 'error',
              ...
              if (editItem.operation === 'add') {
                  this.cartObservableArray.splice(this.cartObservableArray().length, 0, editItem.item.data);
              }
              else {
                  error = {
                      severity: 'error',
       ...
       */
      this.connected = () => {
        accUtils.announce('Cart page loaded.', 'assertive');
        document.title = "Cart";
